...
export interface BentoCardProps {
  ...
  description?: string;
  label?: string;
  icon?: ReactNode; // Added support for icons
  textAutoHide?: boolean;
  disableAnimations?: boolean;
}

export interface BentoProps {
  cards?: BentoCardProps[]; // Added to allow passing dynamic cards
  textAutoHide?: boolean;
  enableStars?: boolean;
...
const BentoCardGrid: React.FC<{
  ...
  <div
    className="bento-section grid gap-2 p-3 w-full max-w-6xl mx-auto select-none relative"
...
const MagicBento: React.FC<BentoProps> = ({
  ...
  // Use default cards if none are provided
      ...
      <BentoCardGrid gridRef={gridRef}>
        <div className="card-responsive grid gap-2">
          {displayCards.map((card, index) => {
            const baseClassName = `card flex flex-col justify-between relative aspect-[4/3] min-h-[200px] w-full max-w-full p-6 rounded-[24px] border border-solid border-gray-100 dark:border-white/10 bg-white dark:bg-[#060010] text-text-main dark:text-white font-light overflow-hidden transition-colors duration-300 ease-in-out hover:-translate-y-0.5 hover:shadow-[0_8px_25px_rgba(0,0,0,0.15)] ${enableBorderGlow ? 'card--border-glow' : ''
              }`;
